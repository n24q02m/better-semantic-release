...
def test_http_status_rejects_non_http_schemes(scheme):
    """
    `_http_status` raises a `ValueError` for any URL scheme other than `http` or `https`
    to prevent SSRF and Local File Inclusion (LFI).
    """
    url = f"{scheme}:///etc/passwd" if scheme else "/etc/passwd"
    with pytest.raises(ValueError, match=f"Invalid URL scheme: {scheme}"):
        reg._http_status(url, timeout=2.0)

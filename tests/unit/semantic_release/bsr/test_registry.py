...
def test_http_status_invalid_scheme():
    """
    `_http_status` raises ValueError on invalid URL scheme to prevent SSRF
    or local file inclusion vulnerabilities.
    """
    with pytest.raises(ValueError, match="Invalid URL scheme: file"):
        reg._http_status("file:///etc/passwd", timeout=2.0)

    with pytest.raises(ValueError, match="Invalid URL scheme: ftp"):
        reg._http_status("ftp://example.com/file", timeout=2.0)

...
def check_python_readme_field(repo_root: Path) -> CheckResult:
    ...
    return CheckResult(
        "Python", "pyproject_readme_field", "PASS" if ok else "FAIL",
...
def _resolve_workspace_members(repo_root: Path, patterns: list[str], exclude_patterns: list[str]) -> list[Path]:
...
def _check_rust_workspace_readme(repo_root: Path, workspace: dict) -> CheckResult:
    members = _resolve_workspace_members(
        repo_root, workspace.get("members") or [], workspace.get("exclude") or [],
    )
    ...
    if not subject:
        detail = "; ".join(excluded_reasons) if excluded_reasons else "no workspace members resolved"
        return CheckResult(
            "Rust", "cargo_readme_field", "SKIP",
    ...
    failing = [
        ...
        if not (isinstance(pkg.get("readme"), str) and pkg.get("readme", "").lower() == "readme.md")
    ]
    if failing:
        detail = "; ".join(f'{name} readme should be "README.md" (got: {readme})' for name, readme in failing)
        return CheckResult(
...
def check_rust_cargo_readme(repo_root: Path) -> CheckResult:
    ...
    return CheckResult(
        "Rust", "cargo_readme_field", "SKIP", "Cargo.toml has neither [package] nor [workspace]",
    )
...
def check_readme_exists(repo_root: Path) -> CheckResult:
    ...
    return CheckResult(
        "Universal", "readme_exists", "PASS", f"{rd.name} ({rd.stat().st_size} bytes)",
    )

...
## 2024-07-26 - Avoid Speculative Optimizations on Stock Upstream Files
**Learning:** A rejected PR taught me that speculative micro-optimizations (like replacing `all([a, b, c])` with `a and b and c` to save a list allocation) are not worth the churn if they aren't on a measured hot path, especially when they are dwarfed by I/O costs like git operations. More importantly, modifying stock `python-semantic-release` files (those outside `src/semantic_release/bsr/` and without `BSR-PATCH` markers) introduces a permanent rebase cost for the fork.
**Action:** Only propose performance PRs with concrete timing/benchmark measurements showing significant gains. Avoid restructuring stock upstream files for unmeasured gains to keep rebases cheap.

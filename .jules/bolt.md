...
## 2026-08-24 - Rejected PR: explicit boolean short-circuiting
**Learning:** The fork maintains a strict 'low-churn policy' to preserve code parity with the upstream repository. Micro-optimizations (e.g., replacing `all([...])` with explicit boolean short-circuiting) will be rejected on upstream code unless they are accompanied by a concrete, reproducible measurement proving a significant performance bottleneck. Do not propose unmeasured structural changes just for theoretical performance gains.
**Action:** Always provide reproducible measurements for optimizations that alter stock upstream code, or focus strictly on fork-owned paths that have demonstrable performance bottlenecks.

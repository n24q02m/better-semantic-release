...
## 2024-08-01 - Negligible Performance Gain vs Maintenance Cost
**Learning:** A speculative micro-optimization replacing `all([a, b, c])` with `a and b and c` was rejected because the performance gain was negligible (the code runs infrequently) and it incurred a permanent rebase cost for upstream synchronization. Unrelated reformatting should also be strictly avoided.
**Action:** Always measure and prove the performance impact on hot paths before submitting an optimization PR. Avoid restructuring stock upstream files for unmeasured gains.

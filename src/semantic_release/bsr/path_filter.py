...
def commit_touches_paths(commit: Commit, paths: tuple[str, ...]) -> bool:
    ...
    # If any prefix is empty after normalization, it matches everything
    if not all(normalized_paths):
        return bool(changed_paths)

    # Performance optimization: str.startswith natively accepts a tuple, executing at C-level speed.
    # We prepare the slash-terminated prefixes once and do O(C * P) checks quickly, rather than O(C * P) loops in Python.
    normalized_paths_slash = tuple(f"{p}/" for p in normalized_paths)

    return any(
        changed_path in normalized_paths
        or changed_path.startswith(normalized_paths_slash)

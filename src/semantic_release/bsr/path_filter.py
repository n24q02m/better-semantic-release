...
def commit_touches_paths(commit: Commit, paths: tuple[str, ...]) -> bool:
    ...
    # Use a tuple for efficient native startswith checking
    prefixes = tuple(f"{p}/" for p in normalized_paths)

    for changed_path in changed_paths:
        # Check exact matches first, then prefixes
        if changed_path in normalized_paths or changed_path.startswith(prefixes):
            return True

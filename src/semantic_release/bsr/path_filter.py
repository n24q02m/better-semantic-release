...
def commit_touches_paths(commit: Commit, paths: tuple[str, ...]) -> bool:
    ...
    # Empty string in paths means "match everything" - optimization to short-circuit
    if "" in normalized_paths and changed_paths:
        return True

    # Performance optimization: str.startswith natively accepts a tuple of strings
    # to execute prefix checks at C-level speed, which is significantly faster
    # than iterating over prefixes in a Python loop.
    prefix_slash = tuple(f"{p}/" for p in normalized_paths)
    return any(
        changed_path in normalized_paths or changed_path.startswith(prefix_slash)

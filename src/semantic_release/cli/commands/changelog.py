...
def changelog(cli_ctx: CliContextObj, release_tag: str | None) -> None:
    ...
    if not isinstance(hvcs_client, RemoteHvcsBase):
        rprint(
            "[bold orange1]:warning: Remote does not support releases. Skipping release notes update...[/]"
        )
        return
        ...
        rprint(
            str.join(
                ...
                [
                    ...
                    f"{rich.markup.escape(repr(translator.tag_format))}[/]",
                ],
            )
        )
        ctx.exit(1)
    ...
    try:
        ...
        rprint(f"[bold red]:x: tag {rich.markup.escape(release_tag)} not in release history[/]")
        ctx.exit(2)
    ...
    try:
        ...
        logger.exception(e)
        rprint("[bold red]:x: Failed to post release notes to remote[/]")
        ctx.exit(1)

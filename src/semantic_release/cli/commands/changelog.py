...
def changelog(cli_ctx: CliContextObj, release_tag: str | None) -> None:
    ...
    if not isinstance(hvcs_client, RemoteHvcsBase):
        rprint(
            "[bold orange1]\\[:warning: WARNING] Remote does not support releases. Skipping release notes update..."
        )
        return
    ...
    if not (version := translator.from_tag(release_tag)):
        ...
        rprint(f"[bold red]\\[:x: ERROR] {rich.markup.escape(msg)}")
        ctx.exit(1)
    ...
    try:
        ...
        rprint(
            f"[bold red]\\[:x: ERROR] tag {rich.markup.escape(str(release_tag))} not in release history"
        )
        ctx.exit(2)
    ...
    try:
        ...
        logger.exception(e)
        rprint("[bold red]\\[:x: ERROR] Failed to post release notes to remote")
        ctx.exit(1)
